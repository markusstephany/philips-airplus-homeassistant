...
- **Reconnect Resilience**: Automatic reconnect with exponential backoff (30 s → 5 min), token *and* authorizer-signature refresh before every attempt, and self-healing from the regular poll cycle so a dropped connection recovers without a manual reload
